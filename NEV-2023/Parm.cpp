...
namespace Parm
{
	PARM getParm(int argc, wchar_t* argv[])
	{
		...
		if (*parm.out == NULL)
		{
			size_t copyLenght = wcslen(parm.in) - 4;
			wcsncpy_s(parm.out, PARM_MAX_SIZE, parm.in, copyLenght);
			wcsncat_s(parm.out, PARM_LOG_DEFAULT_EXT, sizeof(PARM_LOG_DEFAULT_EXT));
		}

		if (*parm.log == NULL)
		{
			size_t copyLenght = wcslen(parm.in) - 4;
			wcsncpy_s(parm.log, PARM_MAX_SIZE, parm.in, copyLenght);
