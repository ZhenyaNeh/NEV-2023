...
namespace Log
{
	...
	void writeError(std::ostream* stream, Error::ERROR error)	// вывод ошибок в логи
	{
		if (stream == NULL)
		{
			if (error.position.col == -1 || error.position.line == -1)
				std::cout << std::endl << "Ошибка N" << error.id << ": " << error.message << std::endl;
			else if (error.position.col == NULL)
				std::cout << std::endl << "Ошибка N" << error.id << ": " << error.message 
				<< " Строка: " << error.position.line << std::endl;
			else
				std::cout << std::endl << "Ошибка N" << error.id << ": " << error.message
				<< " Строка: " << error.position.line
				<< " Позиция в строке: " << error.position.col << std::endl;
			system("pause");
		}
		else
		{
			if (error.position.col == -1 || error.position.line == -1)
				*stream << std::endl << "Ошибка N" << error.id << ": " << error.message;
			else if (error.position.col == NULL)
				*stream << std::endl << "Ошибка N" << error.id << ": " << error.message
				<< " Строка: " << error.position.line;
			else
				*stream << std::endl << "Ошибка N" << error.id << ": " << error.message
				<< " Строка: " << error.position.line
				<< " Позиция в строке: " << error.position.col;
			*stream << std::endl;
		}
	}
