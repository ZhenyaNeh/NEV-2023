...
std::string decimicalNotation(char* dec)	// перевод 16-ного токена в 10-ное число
{
	std::string input = std::string(dec);
...
namespace Lexer
{
	...
	bool analyze(LEX& tables, In::IN& in, Log::LOG& log, Parm::PARM& parm)		// анализ действий при встрече соответствующих лексем
	{
		...
		for (int i = 0; i < In::InWord::size; i++)
		{
			...
			if (curword[0] == 'q')			// перевод из шеснадцатеричной системы 
			{
				bool test = false;
				for (int entry = 0; entry < N_GRAPHS; entry++) 
				{
					FST::FST fst(curword, graphs[entry].graph);
					if(FST::execute(fst))
					{
						std::string resultValue = decimicalNotation(curword);
						strcpy_s(curword, resultValue.c_str());
						test = true;
					}
					if (test)
						break;
					...
					case LEX_SEPARATORS:
					{
						...
						case LEX_LEFTBRACE:		// начало области видимости '{'
						{
							if (i > 0 && *in.words[i - 1].word == LEX_ISFALSE || *in.words[i - 1].word == LEX_ISTRUE || *in.words[i - 1].word == LEX_CYCLE)
								break;
							char* functionname = new char[MAXSIZE_ID];
							char* scopename = getScopeName(tables.idtable, in.words[i - 1].word);	// получаем имя этой области
							if (scopename == nullptr)  
								break;		
							strcpy_s(functionname, MAXSIZE_ID, scopename);
							scopes.push(functionname);
