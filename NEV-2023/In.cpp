...
namespace In
{
	...
	InWord* getWordsTable(std::ostream* stream, unsigned char* text, int* code, int textSize)		// формируем таблицу слов
	{
		...
		for (int i = 0; i < textSize; i++)		// заполнение
		{
			...
			case IN::S:
			{
				/*if (text[i] == LEX_MINUS && isdigit(text[i + 1]) && !isdigit(text[i - 1]))
				{
				...
				bool chekNum = false;
				if (text[i] == LEX_MINUS && isdigit(text[i + 1]))	// проверка на то €вл€етс€ ли минус оператором числа(пример: -2) или  оператором между числами(пример: 4-2)
				{
					for (int k = i - 1; text[k] != '='; k--)
						if (isdigit(text[k])) // числовой литерал с минусом
							chekNum = true;
	...
	void printTable(InWord* table)			// вывод таблицы в консоль
	{
		...
		for (int i = 0; i < table->size; i++)
			std::cout << std::setw(3) << i << std::setw(3) << table[i].line << " |  " << table[i].word << std::endl;
	}
};